...
public class Handler {
	...
	int update1(String find, int kor) {
		for(int i = 0; i < arr.length; i++) {
			if(arr[i] != null && arr[i].name.equals(find)) {
				arr[i].kor = kor;
				arr[i].sum = kor + arr[i].eng + arr[i].mat;
	...
	int update2(String find, int eng) {
		for(int i = 0; i < arr.length; i++) {
			if(arr[i] != null && arr[i].name.equals(find)) {
				arr[i].eng = eng;
				arr[i].sum = arr[i].kor + eng + arr[i].mat;
				arr[i].avg = arr[i].sum/3.0;
				return 1;
			}
		}
	...
	int update3(String find, int mat) {
		for(int i = 0; i < arr.length; i++) {
			if(arr[i] != null && arr[i].name.equals(find)) {
				arr[i].mat = mat;
				arr[i].sum = arr[i].kor + arr[i].eng + mat;
				arr[i].avg = arr[i].sum/3.0;
				return 1;
			}
		}
	...
	public int update4(String find, int kor, int eng, int mat) {
		for(int i = 0 ; i < arr.length; i++) {
			if(arr[i] != null && arr[i].name.equals(find)) {
				arr[i].kor = kor;
				arr[i].eng = eng;
